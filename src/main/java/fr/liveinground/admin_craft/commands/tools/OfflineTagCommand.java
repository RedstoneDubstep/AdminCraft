...
public class OfflineTagCommand {
    ...
    private static int addTagToProfile(CommandSourceStack source, Collection<NameAndId> profiles, String tag) {
        if (profiles.isEmpty()) {
        ...
        if (player != null) {
            ...
            try {
                ...
                if (success == 200) {
                    final NameAndId gp = profile;
                    source.sendSuccess(() -> Component.literal("Added tag '" + tag + "' to offline player " + gp.name()), true);
                } else if (success == 404) {
    ...
    private static int removeTagToProfile(CommandSourceStack source, Collection<NameAndId> profiles, String tag) {
        ...
        if (player != null) {
            ...
            try {
                ...
                if (success == 200) {
                    final NameAndId gp = profile;
                    source.sendSuccess(() -> Component.literal("Removed tag '" + tag + "' to offline player " + gp.name()), true);
                } else if (success == 404) {
                    source.sendFailure(Component.literal("Failure: targeted playerdata file not found").withStyle(ChatFormatting.RED));
                } else if (success == 409) {
    ...
    private static int listProfileTags(CommandSourceStack source, Collection<NameAndId> profiles) {
        ...
        if (player != null) {
            ...
            try {
                List<String> tags = PlayerDataLoader.listOfflineTags(source.getLevel(), profile.id());
                final NameAndId gm = profile;
                if (tags == null) {
                    source.sendSuccess(() -> Component.literal(gm.name() + " has no tags"), false);
                    return 1;
                }
                String builder = "Player " + gm.name() +
                        " has " + tags.size() +
                        " tags: " +
                        String.join(", ", tags);
                source.sendSuccess(() -> Component.literal(builder), false);
